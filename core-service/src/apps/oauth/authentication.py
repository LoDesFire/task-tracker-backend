...
class AuthorizationBackend(BaseBackend):
    ...
    def _get_group_permissions(self, user_obj):
        ...
        return Permission.objects.filter(
            **{user_groups_query: user_obj},
            group__groupscope__scope=GroupPermissionsScope.GLOBAL,
        ).exclude(
            pk__in=restricted_permission_ids,
        )

    @staticmethod
    def _get_user_project_permissions(project_user_obj, restricted=False):
        return Permission.objects.filter(
    ...
    def _get_group_project_permissions(self, project_user_obj):
        restricted_permission_ids = self._get_user_project_permissions(
            project_user_obj,
            restricted=True,
        )
        ...
        return Permission.objects.filter(
            **{user_groups_query: project_user_obj},
            group__groupscope__scope=GroupPermissionsScope.PROJECT,
        ).exclude(
            pk__in=restricted_permission_ids,
        )
    ...
    def _get_project_permissions(
        ...
        if not hasattr(user_obj, perm_cache_name):
            if user_obj.is_superuser:
                ...
                if not project_user_obj.is_active and not user_obj.is_superuser:
                    return set()
    ...
    def get_group_permissions(self, user_obj, obj=None):
        ...
        raise UnknownObject("Unknown permission object type: {}".format(str(type(obj))))
    ...
    def get_all_permissions(self, user_obj, obj=None):
        cache_key = (
            "_perm_cache_{}".format(obj.__class__.__name__)
            if obj
            else "_perm_cache_global"
        )
